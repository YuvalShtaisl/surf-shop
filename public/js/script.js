...
throw new Error("Cannot find module \"my-functions\"");
...
function alignVertical() {
    ...
    $('.align-vertical').each(function () {
        var that = $(this);
        height = that.height(), parentHeight = that.parent().height(), padAmount = parentHeight / 2 - height / 2;
...
jQuery(document).ready(function () {
    ...
    $('.quantity input[type="button"]').on('click', function () {
        ...
        min = qty.attr('min') ? parseInt(qty.attr('min')) : 1, max = qty.attr('max') ? parseInt(qty.attr('max')) : null, step = qty.attr('step') ? parseInt(qty.attr('step')) : 1;
